...
namespace LateToTheParty.Helpers
{
    public static class DatabaseHelpers
    {
        ...
        public static IEnumerable<Item> AssortOfferBaseItems(this TraderAssort assort)
        {
            return assort.Items.Where(item => item.SlotId == HIDEOUT_SLOT_ID);
        }

        public static IEnumerable<Item> AssortOfferChildItems(this TraderAssort assort)
        {
            return assort.Items.Where(item => item.SlotId != HIDEOUT_SLOT_ID);
        }

        public static IEnumerable<Item> GetAllChildItems(this TraderAssort assort, Item parentItem)
        {
            foreach (Item assortItem in assort.AssortOfferChildItems())
            {
                if ((assortItem.ParentId != null) && (assortItem.ParentId == parentItem.Id))
                {
                    yield return assortItem;

                    IEnumerable<Item> childItems = assort.GetAllChildItems(assortItem);
