...
namespace LateToTheParty.Patches
{
    public class OnItemAddedOrRemovedPatch : ModulePatch
    {
        ...
        {
            return typeof(Player).GetMethod(nameof(Player.OnItemAddedOrRemoved), BindingFlags.Public | BindingFlags.Instance);
        }
        ...
        protected static void PatchPostfix(Player __instance, Item item, ItemAddress location, bool added)
        {
            ...
            {
                if (Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.IgnoreItemsDroppedByPlayer.Enabled)
                {
                    preventFromDespawning = true;
                }

                if (Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.IgnoreItemsDroppedByPlayer.OnlyItemsBroughtIntoRaid && item.SpawnedInSession)
                {
