...
namespace LateToTheParty.Helpers.Loot
{
    public static class LootDiscoveryHelpers
    {
        ...
        {
            Type typeToSearch = Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.OnlySearchForNearbyTrunks ? typeof(Trunk) : typeof(WorldInteractiveObject);

            IEnumerable<WorldInteractiveObject> nearbyInteractiveObjects = Singleton<DoorTogglingComponent>.Instance
                .FindNearbyInteractiveObjects(lootInfo.Transform.position, Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.NearbyInteractiveObjectSearchDistance, typeToSearch)
            ...
            if (nearbyInteractiveObjects.Any())
            {
                ...
                Singleton<LoggingUtil>.Instance.LogInfo(lootItem.LocalizedName() + " is nearby " + lootInfo.NearbyInteractiveObject.GetType().Name + " " + lootInfo.NearbyInteractiveObject.Id);
            }
        }

        private static double GetLootFoundTime(double raidET)
        {
            return raidET == 0 ? -1.0 * Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.MinLootAge : raidET;
        }
    }
}
