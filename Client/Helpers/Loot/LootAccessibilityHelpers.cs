...
namespace LateToTheParty.Helpers.Loot
{
    public static class LootAccessibilityHelpers
    {
        public static void UpdateAccessibility(this Item item)
        {
            ...
            if (!Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.CheckLootAccessibility.Enabled)
            {
            ...
            (
                ...
                && (distanceToNearestLockedDoor > Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.CheckLootAccessibility.ExclusionRadius)
            )
            {
                lootInfo.updateLootAccessibility(true, Color.green);
                return;
            ...
            if (Vector3.Distance(nearestPosition, itemPosition) > Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.CheckLootAccessibility.MaxPathSearchDistance)
            {
                return;
            }
