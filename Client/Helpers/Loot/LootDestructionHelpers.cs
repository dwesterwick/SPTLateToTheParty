...
namespace LateToTheParty.Helpers.Loot
{
    public static class LootDestructionHelpers
    {
        ...
        public static void StartDestruction(this Item item)
        {
            ...
            {
                Singleton<LoggingUtil>.Instance.LogError("Could not destroy " + item.LocalizedName());
        ...
        private static void openNearbyDoorForLoot(this Item item)
        {
            ...
            {
                Singleton<LoggingUtil>.Instance.LogError("Cannot open door " + lootInfo.NearbyInteractiveObject.Id + " because its state is " + lootInfo.NearbyInteractiveObject.DoorState);
                return;
            }
        ...
        public static bool IsEligibleForDestruction(this Item item, IEnumerable<Vector3> playerPositions, double raidET)
        {
            ...
            if (lootAge < Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.MinLootAge)
            {
                //Singleton<LoggingUtil>.Instance.LogInfo("Ignoring " + item.LocalizedName() + " (Loot age: " + lootAge + ")");
                return false;
            }
            ...
            if (playerPositions.Any(p => Vector3.Distance(p, lootInfo.Transform.position) < Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.ExclusionRadius))
            {
                return false;
            }
            ...
            if (lootDist < Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.ExclusionRadiusBots)
            {
                return false;
            }

            // Ignore loot that players couldn't have possibly reached yet
            double maxBotRunDistance = raidET * Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.MapTraversalSpeedMps;
            if (maxBotRunDistance < lootInfo.DistanceToNearestSpawnPoint)
            {
                //Singleton<LoggingUtil>.Instance.LogInfo("Ignoring " + item.LocalizedName() + " (Loot Distance: " + LootInfo[item].DistanceToNearestSpawnPoint + ", Current Distance: " + maxBotRunDistance + ")");
                return false;
            }
        ...
        public static void FindChildItemsToDestroy(this Item item, int totalItemsToDestroy, int lootSlotsToDestroy, List<Item> allItemsToDestroy)
        {
            ...
            {
                Singleton<LoggingUtil>.Instance.LogError("Could not get parents of " + item.LocalizedName() + " (" + item.TemplateId + ")");
                throw;
            }
            ...
            {
                Singleton<LoggingUtil>.Instance.LogError("Could not removed excluded items from " + string.Join(",", parentItems.Select(i => i.LocalizedName())));
                throw;
            }
            ...
            if (item.CanRemoveFrom(parentItem))
            {
                ...
                if (allItems.Length > Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.LootRanking.ChildItemLimits.Count)
                {
                    Singleton<LoggingUtil>.Instance.LogInfo(item.LocalizedName() + " has too many child items to destroy.");
                    return;
                }

                double allItemsWeight = allItems.Select(i => i.Weight).Sum();
                if ((allItems.Length > 1) && (allItemsWeight > Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.LootRanking.ChildItemLimits.TotalWeight))
                {
                    Singleton<LoggingUtil>.Instance.LogInfo(item.LocalizedName() + " and its child items are too heavy to destroy.");
                    return;
                }
            ...
            Singleton<LoggingUtil>.Instance.LogInfo(item.LocalizedName() + " cannot be removed from " + parentItem.LocalizedName() + ". Destroying parent item and all children.");
        ...
        public static bool TryAddItemToDespawnList(this Item item, Item parentItem, List<Item> allItemsToDestroy)
        {
            ...
            if (lootInfo == null)
            {
                Singleton<LoggingUtil>.Instance.LogWarning("Could not find entry for " + item.LocalizedName());
                return false;
            }

            if (lootInfo.CannotBeDestroyed)
            {
                //Singleton<LoggingUtil>.Instance.LogWarning("Ensure parent " + parentItem.LocalizedName() + " for " + item.LocalizedName() + " is being destroyed!");
                return false;
            }

            if (item.CurrentAddress == null)
            {
                Singleton<LoggingUtil>.Instance.LogWarning("Invalid parent for " + item.LocalizedName());
                return false;
            }
