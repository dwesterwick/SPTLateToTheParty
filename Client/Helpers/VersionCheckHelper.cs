    ...
    public class VersionCheckHelper
    {
        ...
        public static bool IsSPTWithinVersionRange(string minVersionString, string maxVersionString, out string currentVersionString)
        {
            ...
            try
            {
                ...
                if (assembly == null)
                {
                    Singleton<LoggingUtil>.Instance.LogError("Could not find assembly " + sptCommonAssemblyName);
                    return false;
                }
            ...
            {
                Singleton<LoggingUtil>.Instance.LogError("An exception occurred when checking the current SPT version: " + e.Message);
                return false;
            }
