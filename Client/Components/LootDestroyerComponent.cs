...
namespace LateToTheParty.Components
{
    public class LootDestroyerComponent : MonoBehaviour
    {
        ...
        protected void Update()
        {
            ...
            if (updateTimer.ElapsedMilliseconds < Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.MinTimeBeforeUpdateMs)
            {
                return;
            }
            ...
            if (
                ...
                && (updateTimer.ElapsedMilliseconds < Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.MaxTimeBeforeUpdateMs)
                && (maxDistanceTravelledByPlayers < Singleton<ConfigUtil>.Instance.CurrentConfig.DestroyLootDuringRaid.MinDistanceTraveledForUpdate)
            )
            {
                return;
            }
        ...
        {
            return Singleton<ConfigUtil>.Instance.CurrentConfig.OnlyMakeChangesJustAfterSpawning.Enabled
                && Singleton<ConfigUtil>.Instance.CurrentConfig.OnlyMakeChangesJustAfterSpawning.AffectedSystems.LootDestruction
                && LootManager.HasInitialLootBeenDestroyed
                && (lootDestructionTimer.ElapsedMilliseconds / 1000.0 > Singleton<ConfigUtil>.Instance.CurrentConfig.OnlyMakeChangesJustAfterSpawning.TimeLimit);
        }
    }
}
