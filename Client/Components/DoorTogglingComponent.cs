...
namespace LateToTheParty.Components
{
    public class DoorTogglingComponent : MonoBehaviour
    {
        ...
        private EnumeratorWithTimeLimit enumeratorWithTimeLimit = new EnumeratorWithTimeLimit(Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.MaxCalcTimePerFrameMs);
        ...
        {
            if (!Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.Enabled)
            {
        ...
        protected void Update()
        {
            ...
            if (HasToggledInitialInteractiveObjects && (updateTimer.ElapsedMilliseconds < Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.TimeBetweenDoorEvents * 1000))
            {
                return;
            }
            ...
            InteractiveObjectsToToggle = (int)Math.Max(1, Math.Round(eligibleInteractiveObjects.Count * Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.PercentageOfDoorsPerEvent / 100.0));
            if (!HasToggledInitialInteractiveObjects)
            {
                InteractiveObjectsToToggle *= (int)Math.Ceiling(Math.Max(raidTimeElapsed - Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.MinRaidET, 0) / Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.TimeBetweenDoorEvents);
            ...
            if ((raidTimeElapsed < Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.MinRaidET) || (raidTimeRemaining < Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.MinRaidTimeRemaining))
            {
                if (!HasToggledInitialInteractiveObjects)
                {
                    Singleton<LoggingUtil>.Instance.LogInfo("Interactive objects cannot be opened at this time in the raid");
                    HasToggledInitialInteractiveObjects = true;
                }
        ...
        public bool ToggleInteractiveObject(WorldInteractiveObject interactiveObject, EDoorState newState)
        {
            ...
            if ((interactiveObject.DoorState == EDoorState.Locked) && (newState == EDoorState.Open))
            {
                if (interactiveObject.KeyId.Length > 0)
                {
                    ...
                    if (shouldlimitEvents() && Singleton<ConfigUtil>.Instance.CurrentConfig.OnlyMakeChangesJustAfterSpawning.AffectedSystems.OpeningLockedDoors)
                    {
                        return true;
                    }
                    ...
                    if (randomObj.Next(0, 100) > Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.ChanceOfUnlockingDoors)
                    {
                        return false;
                    }

                    Singleton<LoggingUtil>.Instance.LogInfo("Preparing to unlock interactive object: " + interactiveObject.Id + " (Key ID: " + interactiveObject.KeyId + ")");
                }
                else
                {
                    // Skip the event if changes need to be limited after a certain time has elapsed after spawning
                    if (shouldlimitEvents() && Singleton<ConfigUtil>.Instance.CurrentConfig.OnlyMakeChangesJustAfterSpawning.AffectedSystems.OpeningUnlockedDoors)
                    {
                        return true;
                    }
                    ...
                    {
                        Singleton<LoggingUtil>.Instance.LogInfo("Preparing to breach door: " + door.Id);
                    }
                    else
                    {
            ...
            {
                // Skip the event if changes need to be limited after a certain time has elapsed after spawning
                if (shouldlimitEvents() && Singleton<ConfigUtil>.Instance.CurrentConfig.OnlyMakeChangesJustAfterSpawning.AffectedSystems.ClosingDoors)
                {
                    return true;
                }
        ...
        private IEnumerator ToggleRandomInteractiveObjects(int interactiveObjectsToToggle)
        {
            try
            {
                ...
                enumeratorWithTimeLimit.Reset();
                yield return enumeratorWithTimeLimit.Repeat(interactiveObjectsToToggle, ToggleRandomInteractiveObject, doorsThatCanBeToggled, Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.MaxCalcTimePerFrameMs);
            }
            finally
            {
        ...
        private IEnumerator FindAllEligibleInteractiveObjects()
        {
            try
            {
                ...
                Singleton<LoggingUtil>.Instance.LogInfo("Searching for valid interactive objects...");
                ...
                Singleton<LoggingUtil>.Instance.LogInfo("Searching for valid interactive objects...found " + allDoors.Count() + " possible interactive objects.");

                enumeratorWithTimeLimit.Reset();
                yield return enumeratorWithTimeLimit.Run(allDoors, CheckIfInteractiveObjectIsEligible);

                Singleton<LoggingUtil>.Instance.LogInfo("Searching for valid interactive objects...found " + eligibleInteractiveObjects.Count + " interactive objects.");
            }
            finally
            {
        ...
        private void CheckIfInteractiveObjectIsEligible(WorldInteractiveObject interactiveObject)
        {
            ...
            foreach (NoPowerTip noPowerTip in allNoPowerTips)
            {
                ...
                Singleton<LoggingUtil>.Instance.LogDebug("Found NoPowerTip " + noPowerTip.name + " for door " + interactiveObject.Id);
        ...
        private bool IsInteractiveObjectAllowedToBeToggled(WorldInteractiveObject interactiveObject)
        {
            ...
            {
                Singleton<LoggingUtil>.Instance.LogInfo("NoPowerTip for door " + interactiveObject.Id + " is still active.");
                return false;
            }
        ...
        private bool IsEligibleInteractiveObject(WorldInteractiveObject interactiveObject, bool logResult = false)
        {
            ...
            {
                if (allItems.ContainsKey(interactiveObject.KeyId) && !Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.CanOpenLockedDoors)
                {
                    if (logResult) Singleton<LoggingUtil>.Instance.LogDebug("Searching for valid interactive objects...interactive object " + interactiveObject.Id + " is locked and not allowed to be opened.");
                    return false;
                }

                Door? door = interactiveObject as Door;
                if ((door?.CanBeBreached == true) && !Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.CanBreachDoors)
                {
                    if (logResult) Singleton<LoggingUtil>.Instance.LogDebug("Searching for valid interactive objects...door " + door.Id + " is not allowed to be breached.");
                    return false;
                }
            }
        ...
        private bool CheckIfInteractiveObjectCanBeToggled(WorldInteractiveObject interactiveObject, bool logResult = false)
        {
            ...
            {
                if (logResult) Singleton<LoggingUtil>.Instance.LogDebug("Searching for valid interactive objects...interactive object " + interactiveObject.Id + " is inoperable.");
                return false;
            }

            if (interactiveObject.gameObject.layer != LayerMask.NameToLayer("Interactive"))
            {
                if (logResult) Singleton<LoggingUtil>.Instance.LogDebug("Searching for valid interactive objects...interactive object " + interactiveObject.Id + " is inoperable (wrong layer).");
                return false;
            }
            ...
            {
                if (logResult) Singleton<LoggingUtil>.Instance.LogDebug("Searching for valid interactive objects...interactive object " + interactiveObject.Id + " has an invalid state: " + interactiveObject.DoorState);
                return false;
            }
            ...
            if (interactiveObject.DoorState == EDoorState.Locked)
            {
                ...
                {
                    if (logResult) Singleton<LoggingUtil>.Instance.LogDebug("Searching for valid interactive objects...door " + door.Id + " is locked and has no valid key.");
                    return false;
                }
            }
        ...
        private void ToggleRandomInteractiveObject(IEnumerable<WorldInteractiveObject> eligibleInteractiveObjects, float maxCalcTime_ms)
        {
            ...
            while (calcTimer.ElapsedMilliseconds < maxCalcTime_ms)
            {
                ...
                if (randomObj.Next(0, 100) < Singleton<ConfigUtil>.Instance.CurrentConfig.OpenDoorsDuringRaid.ChanceOfClosingDoors)
                {
                ...
                {
                    //Singleton<LoggingUtil>.Instance.LogInfo("Attempting to change interactive object " + door.Id + " to " + newState + "...");
        ...
        private bool shouldlimitEvents()
        {
            bool shouldLimit = HasToggledInitialInteractiveObjects
                && Singleton<ConfigUtil>.Instance.CurrentConfig.OnlyMakeChangesJustAfterSpawning.Enabled
                && (interactiveObjectOpeningsTimer.ElapsedMilliseconds / 1000.0 > Singleton<ConfigUtil>.Instance.CurrentConfig.OnlyMakeChangesJustAfterSpawning.TimeLimit);
